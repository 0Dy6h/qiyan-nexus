...
- 门禁全红时先排除工具链再改代码：pnpm 写绝对 symlink，仓库目录一旦移动，前端依赖全部悬空、前端门禁全红且与 diff 无关，需 `rm -rf frontend/node_modules && pnpm install --frozen-lockfile`。判断某条失败是否既有，用 `git stash` 清空改动后复跑确认，不要凭印象归因。
