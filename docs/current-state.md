...
**当前唯一推荐工程主线（2026-07-11）**：完成 `PdfUploadRecord` owner isolation 纵向切片。闭环必须同时覆盖 upload record、download、parse/status、literature search/detail projection、uploaded chunk、RAG retrieval/citation 与 SSR reviewer forwarding；legacy ownerless runtime 数据应隔离或 fail closed。不要只在上传入口增加 `owner_id`，否则后续查询和引用仍会越权。
...
**历史候选记录（非当前优先级）**：下列条目保留 2026-05/06 的完成事实与治理债；下一工程任务必须遵循本节首行的 PDF owner-isolation 主线。
...
7. 历史治理候选（均不覆盖当前 PDF owner-isolation 主线）：
