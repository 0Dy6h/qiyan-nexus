...
- 数据来源切换面板（B6）：**2026-06-10 复核已落地**，不再作为待办。实现来自 `fcf9ac4`：`/literature` 搜索表单使用 `LiteratureDataSourceView` 四选项（全部来源 / PubMed 实时 / CNKI sample / 上传 PDF），`LiteratureDataSourceBanner` 随 view 切换合规文案与 tone；后端 `has_pdf_upload` 查询过滤可筛上传 PDF 文献；前端 `frontend/tests/literature-data-source-switcher.test.ts` 与后端 `backend/tests/test_literature_search.py` 已覆盖。详见 `docs/handoffs/2026-06-10-multilingual-embedding-spike-b6.md`。
