...
- LLM claim-quality v2 live validation：**2026-06-02 已用真实 key 重新采样 10 个问题**（`docs/evaluations/2026-06-02-claim-quality-v2-live-validation.md`）。配置为 `opencode_go + keyword + bge + semantic_threshold=0.3 + transformers NLI=0.5`；14/14 条 claim 均为单 evidence ref，0 条无 ref，0 条多 ref，0 条 unsupported ref/schema parse failure；4/10 个回答 passed，6/10 个回答 blocked，拦截原因均为 `nli_low_entailment`。快速 claim-level review 显示 4 个 passed 回答的 claim 与其 cited chunk 直接对齐，但这不是正式医生/科研 reviewer sign-off。结论：v2 明显改善结构化 claim 质量，L1 受控 smoke/demo 路径更可用；**L2/default preview 仍不翻转**，默认仍为 deterministic。
...
最新项目级状态见 `docs/evaluations/2026-06-02-claim-quality-v2-live-validation.md`（✅ claim-quality v2 真实采样：10 题、14 条单 evidence-ref claims、4 passed / 6 NLI blocked）、`docs/evaluations/2026-06-01-nli-real-distribution.md`（✅ NLI 真实分布评估：0 FP, 0 FN, gap +0.9549）、`docs/evaluations/2026-05-31-opencode-go-bge-smoke.md`（✅ 真实 LLM live smoke）、`docs/adr/0012-real-llm-enablement.md`、`docs/guides/real-llm-enablement-runbook.md`。**L2 推进** 工程前置与 claim-quality v2 真实技术验证已完成；阻塞项转为正式 reviewer 对 passed claims 的人工核验、BGE=0.3 评估 profile 是否可接受的治理决策、以及真实价格 SLI。近期候选方向包括：
...
3. **L2 默认预览推进（工程部分✅，走查完成，决策不翻转）**：
   ...
   - ✅ **claim-quality v2 live validation**（2026-06-02）：BGE 预筛降至 0.3 后，NLI gate 放行 4/10 个回答；所有 14 条 claim 均单证据引用，未见 raw draft 泄漏。决策仍不翻转 L2。
...
7. 下一步候选：
   ...
   - ② ~~BGE 阈值 + NLI gate 的真实 LLM 重验证~~ → **技术采样已完成**（BGE=0.3 + NLI=0.5 下 4/10 passed）；仍需正式 reviewer 核验 passed claims 与治理判断。
   - ③ ~~LLM claim 质量控制（结构化 claim 校验、grounding gate 增强）~~ → **prompt/schema 质控 v2 已落地并 live 验证结构改善**；后续重点是 reviewer sign-off、真实价格 SLI、是否接受 lower-BGE-prefilter profile。
