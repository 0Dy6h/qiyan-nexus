...
def test_answer_question_returns_ranked_citation_cards_for_gut_skin_axis_question():
    ...
    literally contains ``肠-脑-皮肤轴`` (the exact query topic), and the rare single-CJK
    tokens ``脑``/``轴`` get high IDF values amplified by the title field weight (×3).
    microbiome-003 is about gut microbiome dysbiosis — related but not the same topic —
    and drops to #2. Both docs remain top-2; rag-eval-001 lists both as
    expected_literature for the gut-axis theme.
    """
...
def test_answer_question_includes_relevant_citations_for_english_barrier_question():
    ...
    for an English query. Ranking is still deterministic, so we pin the order and
    assert the barrier-specific paper is retrieved.
    """
    response = answer_question("atopic dermatitis barrier")
...
def test_answer_question_returns_empty_related_entity_ids_when_literature_has_none():
    """gbs-001 (IDF-weighted top-1 for this query) also has no literature-level
...
def test_answer_question_swaps_to_opencode_go_provider_via_env(monkeypatch):
    ...
    # claim text overlaps gbs-001 chunk wording (IDF-weighted top-1). Set the
    ...
    # After IDF-weighted scoring the top-1 chunk for this query is
    # chunk-cn-ad-gbs-001-abstract (see
    # test_answer_question_returns_ranked_citation_cards_for_gut_skin_axis_question
...
def test_answer_question_uses_opencode_go_native_tool_claims(monkeypatch):
    ...
    # so the IDF-weighted top-1 chunk swap doesn't fail semantic overlap.
    monkeypatch.setenv("QIYAN_GROUNDING_SEMANTIC_THRESHOLD", "0")
    monkeypatch.setattr(
        opencode_go_provider.OpenCodeGoProvider,
        "generate_answer",
    ...
    # IDF-weighted scoring: chunk-cn-ad-gbs-001-abstract is the top-1 evidence ref.
    assert response.answer == (
